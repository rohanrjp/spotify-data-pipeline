...
WITH weekly_stats AS (
    SELECT 
        song_name, artist_name, play_week as period_date, 'week' as grain,
...
monthly_stats AS (
    SELECT 
        song_name, artist_name, play_month as period_date, 'month' as grain,
        COUNT(*) as play_count, ROUND(SUM(duration_ms) / 60000.0,2) as minutes_played
    FROM {{ ref('stg_recently_played_at') }}
    GROUP BY 1, 2, 3
),
yearly_stats AS (
    SELECT 
        song_name, artist_name, play_year as period_date, 'year' as grain,
        COUNT(*) as play_count, ROUND(SUM(duration_ms) / 60000.0,2) as minutes_played
    FROM {{ ref('stg_recently_played_at') }}
    GROUP BY 1, 2, 3
)
